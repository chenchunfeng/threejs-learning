...
void main(){
    ...
    // vec3 mixColor = mix(redColor, yellowColor, vUv.x);
    ...
    //8 利用step(edge, x)如果x < edge，返回0.0，否则返回1.0 非黑即白
    // float strength =  mod(vUv.y * 10.0 , 1.0) ;
    ...
    // 20 利用取整，实现条纹渐变，换个方向，再换个渐变颜色
    // float strength = floor(vUv.y * 10.0) / 10.0;
    // vec3 redColor = vec3(1, 0, 0);
    // vec3 yellowColor = vec3(1, 1, 0);
    // vec3 mixColor = mix(redColor, yellowColor, strength);
    // gl_FragColor =vec4(mixColor, 1);
    ...
    // 25 依据length返回向量长度
    // float strength = length(vUv);  // 返回向量x的长度
    // gl_FragColor =vec4(strength,strength,strength,1);

    // 26 根据distance计算2个向量的距离
    // float strength =1.0 - distance(vUv,vec2(0.5,0.5));
    // gl_FragColor =vec4(strength,strength,strength,1);

    // 27 根据相除，实现大太阳
    // float strength = 0.15 / distance(vUv,vec2(0.5,0.5)) - 0.5;
